...
当前阶段已开始接口测试第一阶段适配。接口测试先做文档解析、手动修正、样例编辑和 Excel 导出，不接 AI、不做自动化脚本生成。
...
- 接口测试第一阶段 MVP
  ...
  - 支持解析结果手动修正
  - 支持基于解析结果生成 3 条接口测试样例
  - 支持样例 ApiTestCase 表格编辑、接口 Excel 导出
  - 暂不接 AI、不做规则生成、不生成 pytest 脚本、不接 RAG
- 优化规则生成结果可读性
  - 长需求片段不再直接作为用例标题
...
10. 接口文档解析 MVP
   ...
   - 生成的接口样例会引用解析出的参数、请求体、鉴权、业务规则和数据库校验

下一步候选：

- 接口测试第二阶段：从 `ApiDocument` 规则生成更像真实测试设计的接口用例，覆盖正常、异常、鉴权、边界、数据库校验
- 接口测试质量检查：接口字段完整性、断言完整性、状态码/响应体/数据库校验覆盖提示

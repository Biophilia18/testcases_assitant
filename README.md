...
- 功能测试：需求解析、功能点预览、规则/AI 生成、用例编辑、质量检查、需求规则覆盖追踪、导出前复核清单、JSON 保存、Excel 导出。
- 接口测试：单接口文档解析、接口清单识别、文档可信度提示、参数风险分析、规则生成、用例编辑、质量检查、导出前复核清单、Excel 导出。
- 接口自动化辅助：可导出 `api_auto` YAML 草稿，但不承诺直接可执行。
- 稳定性保障：接口示例快照回归、接口用例去重与策略上限、功能点识别降噪提示。
...
接口生成会按策略控制规模：精简最多 10 条，标准最多 18 条，完整最多 35 条；同一风险来源和风险类型的重复用例会自动合并并重新编号。
...
功能测试质量报告会追踪需求文本中的“验收标准”和“补充规则/异常场景”，展示每条规则是否被当前用例命中。该结果用于辅助复核，不能替代测试人员对业务规则的最终判断。

导出前复核清单会在下载 Excel 前汇总明显错误、覆盖缺口和人工复核建议。该清单不强制阻断导出，但建议优先处理错误和警告后再交付或执行。
